...
/**
* @brief To afficher setting .
* @param menu the struct menu
* @param ecran the screen
* @param x the position.x in the screen
* @param y the position.y in the screen
* @return Nothing
*/
...
/**
* @brief To afficher sousmenuingame .
* @param menu the struct menu
* @param ecran the screen
* @param x the position.x in the screen
* @param y the position.y in the screen
...
void initialiser_menu(menu *menu){
	...
	menu->positionbplay.x=420;
	menu->positionbplay.y=180;

	menu->positiondroit.x=700;
	menu->positiondroit.y=255;

	menu->positiongauche.x=340;
...
int affcihier(menu *menu,SDL_Surface *ecran){
	...
	if(menu->po==2){
		...
		else if ((719<=x)&&(x<=788)&&(y<=384)&&(274<=y)){
			SDL_BlitSurface(menu->background, NULL, ecran, &menu->positionecran);
		...
		else if ((360<=x)&&(x<=424)&&(y<=381)&&(274<=y)){
			SDL_BlitSurface(menu->background, NULL, ecran, &menu->positionecran);
			SDL_BlitSurface(menu->bsetting,NULL,ecran,&menu->positionbplay);
			SDL_BlitSurface(menu->droit, NULL, ecran, &menu->positiondroit);
